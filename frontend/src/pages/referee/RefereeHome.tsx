...
export type RefereeScheduleRace = {
  id: string | number;
  time: string;
  race: string;
  details: string;
...
export type RefereeHomeData = {
  heading?: string;
  subheading?: string;
  assignedCount?: number;
  completedCount?: number;
  liveRace?: { series: string; race: string; participants?: string; dateTime?: string; distance?: string; imageUrl?: string };
  notifications?: Array<{ id: string | number; title: string; message: string }>;
  weatherWarning?: { title: string; message: string; countdown?: string };
  schedule?: RefereeScheduleRace[];
};
...
type RefereeHomeProps = { data?: RefereeHomeData | null; loading?: boolean; error?: string };

export const RefereeHome: React.FC<RefereeHomeProps> = ({ data, loading = false, error }) => {
    ...
    <RefereeShell>
      <section className="referee-home-heading">
        <div>
          ...
          <p>{data?.subheading || 'Referee workspace overview'}</p>
        </div>
        <div className="referee-home-stats" aria-label="Race assignment summary">
          <div>
            <span>Assigned</span>
            <strong>{data?.assignedCount ?? 0}</strong>
          </div>
          <div>
            <span>Completed</span>
      ...
      <section className="referee-home-grid">
        {data?.liveRace ? <article className="referee-live-card">
          <header>
            <TimerIcon />
            <span>Live Now</span>
          </header>
          <div className="referee-live-card__media">
            <img src={data.liveRace.imageUrl || raceImage} alt="Current assigned race" />
            <div className="referee-live-card__overlay">
              <div className="referee-live-card__title">
                <div>
                  <span>{data.liveRace.series}</span>
                  <h2>{data.liveRace.race}</h2>
                </div>
                <div className="referee-live-card__participants">
                  <UsersIcon />
                  <div>
                    <span>Participants</span>
                    <strong>{data.liveRace.participants || 'Pending data'}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="referee-live-card__details">
                <div>
                  ...
                  <dl>
                    <dt>Date &amp; Time</dt>
                    <dd>{data.liveRace.dateTime || 'Pending data'}</dd>
                  </dl>
                </div>
                <div>
                  <RulerIcon />
                  <dl>
                    <dt>Distance</dt>
                    <dd>{data.liveRace.distance || 'Pending data'}</dd>
                  </dl>
                </div>
                <Link to={`/Referee/Races/${encodeURIComponent(data.liveRace.race)}`} className="referee-live-card__button">
                  View Race Details <ArrowIcon />
                </Link>
              </div>
            </div>
          </div>
        </article> : <div className="referee-home-empty">No live race data is available.</div>}

        <aside className="referee-side-stack">
          <article className="referee-notifications">
            <h2>Recent Notifications</h2>
            {(data?.notifications ?? []).map((notification) => <div className="referee-notification" key={notification.id}><BriefcaseIcon /><div><strong>{notification.title}</strong><p>{notification.message}</p></div></div>)}
            {!data?.notifications?.length ? <div className="referee-home-empty">No recent notifications.</div> : null}
            <Link to="/Referee/Notifications">View All Notifications</Link>
          </article>

          {data?.weatherWarning ? <article className="referee-weather">
            <div>
              <h2>{data.weatherWarning.title}</h2>
              <p>{data.weatherWarning.message}</p>
              {data.weatherWarning.countdown ? <strong>{data.weatherWarning.countdown}</strong> : null}
            </div>
            <WindIcon />
          </article> : null}
        </aside>
      ...
      <section className="referee-schedule">
        ...
        <div className="referee-schedule__table" role="table" aria-label="Assigned race schedule">
          ...
          {loading ? <div className="referee-home-empty">Loading referee data...</div> : error ? <div className="referee-home-empty">{error}</div> : schedule.length ? schedule.map((race) => (
            <div className="referee-schedule__row" role="row" key={race.id}>
              ...
              <Link to={`/Referee/Races/${encodeURIComponent(race.race)}`}>{race.action}</Link>
            </div>
          )) : <div className="referee-home-empty">No assigned race schedule is available.</div>}
        </div>
      </section>
    </RefereeShell>
  );
