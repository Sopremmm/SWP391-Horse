...
export type RaceStatus = 'Live Now' | 'Upcoming' | 'Completed';
...
type RefereeRacesProps = { races?: AssignedRace[]; loading?: boolean; error?: string };

export const RefereeRaces: React.FC<RefereeRacesProps> = ({ races = [], loading = false, error }) => {
  const [filter, setFilter] = React.useState<'All' | 'Upcoming' | 'History'>('All');
  const visibleRaces = races.filter((race) => filter === 'All' || (filter === 'History' ? race.status === 'Completed' : race.status !== 'Completed'));
  const completed = races.filter((race) => race.status === 'Completed').length;
  const upcoming = races.length - completed;
  return (
    <RefereeShell>
      <section className="referee-races-page">
        ...
        <nav className="referee-races-tabs" aria-label="Race filters">
          {(['All', 'Upcoming', 'History'] as const).map((tab) => (
            <button type="button" className={filter === tab ? 'is-active' : undefined} onClick={() => setFilter(tab)} key={tab}>
              {tab}
            </button>
          ))}
        </nav>
        ...
        <section className="referee-races-card" aria-label="Assigned races table">
          <div className="referee-races-table" role="table">
            ...
            {loading ? <div className="referee-races-empty">Loading assigned races...</div> : error ? <div className="referee-races-empty">{error}</div> : visibleRaces.length ? visibleRaces.map((race) => (
              <div className="referee-races-row" role="row" key={race.id}>
            ...
            )) : <div className="referee-races-empty">No assigned race data is available.</div>}
          </div>
        </section>

        <div className="referee-races-pagination">
          <span>Showing {visibleRaces.length} of {races.length} races</span>
        </div>

        <section className="referee-races-summary" aria-label="Race assignment totals">
          <SummaryCard tone="green" label="Total Assigned" value={String(races.length)} icon={<ClipboardIcon />} />
          <SummaryCard tone="neutral" label="Completed" value={String(completed)} icon={<CheckIcon />} />
          <SummaryCard tone="warm" label="Upcoming" value={String(upcoming)} icon={<TrayIcon />} />
        </section>
      </section>
    </RefereeShell>
  );
