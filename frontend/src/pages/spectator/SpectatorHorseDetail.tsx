...
// Legacy asset mappings are intentionally not used as runtime fallbacks; API data must provide the profile.
void runningHorse;
void profileBySlug;
void raceHistory;
void slugify;
...
type SpectatorHorseDetailProps = { profile?: HorseProfile | null; raceHistory?: RaceHistoryRow[]; loading?: boolean; error?: string };

export const SpectatorHorseDetail: React.FC<SpectatorHorseDetailProps> = ({ profile, raceHistory = [], loading = false, error }) => {
  const { name } = useParams();

  if (!profile) return <div className="spectator-horse-detail-page"><SpectatorHeader showFunds={false} showNotifications /><main className="spectator-api-empty">{loading ? 'Loading horse profile...' : error || `No profile data is available for ${titleCaseFromParam(name, 'this horse')}.`}</main><SpectatorFooter /></div>;
