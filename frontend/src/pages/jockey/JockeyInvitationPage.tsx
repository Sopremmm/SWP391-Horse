...
type Props = { invitations?: JockeyInvitation[] | null; loading?: boolean };
type Filter = 'all' | 'pending' | 'accepted';

export default function JockeyInvitationPage({ invitations, loading = false }: Props) {
  ...
  return <div className="jockey-invitation"><JockeyHeader /><main className="jockey-invitation__main" aria-busy={loading}>
    ...
    <nav className="jockey-invitation__tabs" aria-label="Invitation status filters">
      {([['all','All'],['pending','Pending'],['accepted','Accepted']] as [Filter,string][]).map(([value,label]) => <button className={filter === value ? 'is-active' : ''} onClick={() => setFilter(value)} key={value}>{label}<span>{value === 'all' ? items.length : count(value)}</span></button>)}
    </nav>
    <div className="jockey-invitation__layout"><section className="jockey-invitation__list">
      {visible.length ? visible.map((invite) => <article className="jockey-invitation__card" key={invite.id}><div><span className={`is-${invite.status}`}>{invite.status}</span><h2>{invite.horseName}</h2><p>{[invite.stableName, invite.tournamentName].filter(Boolean).join(' • ')}</p><time>▣ {invite.raceDate || 'Date pending'}</time></div><Link to={`/Jockey/Invitation/${encodeURIComponent(invite.id)}`}>View Details</Link></article>) : <div className="jockey-invitation__empty">No invitations found for this status.</div>}
