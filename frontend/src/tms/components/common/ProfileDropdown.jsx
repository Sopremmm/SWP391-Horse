import React from 'react';
import { Link, useNavigate } from 'react-router-dom';
import './ProfileDropdown.css';

const UserIcon = () => (
  <svg className="profile-dropdown__user-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path
      d="M12 12C10.9 12 9.95833 11.6083 9.175 10.825C8.39167 10.0417 8 9.1 8 8C8 6.9 8.39167 5.95833 9.175 5.175C9.95833 4.39167 10.9 4 12 4C13.1 4 14.0417 4.39167 14.825 5.175C15.6083 5.95833 16 6.9 16 8C16 9.1 15.6083 10.0417 14.825 10.825C14.0417 11.6083 13.1 12 12 12ZM4 20V17.2C4 16.6333 4.14583 16.1125 4.4375 15.6375C4.72917 15.1625 5.11667 14.8 5.6 14.55C6.63333 14.0333 7.68333 13.6458 8.75 13.3875C9.81667 13.1292 10.9 13 12 13C13.1 13 14.1833 13.1292 15.25 13.3875C16.3167 13.6458 17.3667 14.0333 18.4 14.55C18.8833 14.8 19.2708 15.1625 19.5625 15.6375C19.8542 16.1125 20 16.6333 20 17.2V20H4Z"
      fill="currentColor"
    />
  </svg>
);

const ProfileDropdown = () => {
  const navigate = useNavigate();
  const [open, setOpen] = React.useState(false);

  const handleBlur = (event) => {
    if (!event.currentTarget.contains(event.relatedTarget)) {
      setOpen(false);
    }
  };

  const handleLogout = () => {
    navigate('/');
  };

  return (
    <div className="profile-dropdown" onBlur={handleBlur}>
      <button
        type="button"
        className="profile-dropdown__trigger"
        aria-haspopup="menu"
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
      >
        <UserIcon />
      </button>

      {open ? (
        <div className="profile-dropdown__menu" role="menu">
          <Link className="profile-dropdown__item" role="menuitem" to="/HorseOwner/Profile" onClick={() => setOpen(false)}>
            Profile
          </Link>
          <Link className="profile-dropdown__item" role="menuitem" to="/HorseOwner/MyHorses" onClick={() => setOpen(false)}>
            My Horses
          </Link>
          <Link className="profile-dropdown__item" role="menuitem" to="/HorseOwner/HireJockeys" onClick={() => setOpen(false)}>
            Hire Jockeys
          </Link>
          <Link className="profile-dropdown__item" role="menuitem" to="/HorseOwner/MyTournament" onClick={() => setOpen(false)}>
            My Tournament
          </Link>
          <button type="button" className="profile-dropdown__item profile-dropdown__item--logout" role="menuitem" onClick={handleLogout}>
            Logout
          </button>
        </div>
      ) : null}
    </div>
  );
};

export default ProfileDropdown;
