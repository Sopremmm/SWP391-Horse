...
CREATE TABLE dbo.jockey_invitation (
    ...
    tournament_id BIGINT NOT NULL CONSTRAINT FK_jockey_invitation_tournament FOREIGN KEY REFERENCES dbo.tournament(id) ON DELETE CASCADE,
    ...
    CONSTRAINT UQ_jockey_invitation UNIQUE (horse_id, jockey_id, tournament_id)
);

-- Compatibility upgrade for databases created by an older schema. Invitations
-- are tournament-scoped now; keep race_id only for legacy rows and make it optional.
IF OBJECT_ID(N'dbo.jockey_invitation', N'U') IS NOT NULL
BEGIN
    IF COL_LENGTH(N'dbo.jockey_invitation', N'tournament_id') IS NULL
    BEGIN
        ALTER TABLE dbo.jockey_invitation ADD tournament_id BIGINT NULL;
        ALTER TABLE dbo.jockey_invitation ADD CONSTRAINT FK_jockey_invitation_tournament
            FOREIGN KEY (tournament_id) REFERENCES dbo.tournament(id);
    END;

    IF COL_LENGTH(N'dbo.jockey_invitation', N'race_id') IS NOT NULL
       AND COLUMNPROPERTY(
            OBJECT_ID(N'dbo.jockey_invitation'),
            N'race_id',
            'AllowsNull'
       ) = 0
        ALTER TABLE dbo.jockey_invitation ALTER COLUMN race_id BIGINT NULL;
END;
