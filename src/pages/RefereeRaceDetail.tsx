...
const participants: Participant[] = [
  { gate: '01', horse: 'Golden Sovereign', breed: 'Thoroughbred - Stallion', jockey: 'Marcus Thorne' },
  { gate: '02', horse: 'Midnight Breeze', breed: 'Arabian - Mare', jockey: 'Sarah Jenkins' },
  { gate: '03', horse: 'Storm Chaser', breed: 'Thoroughbred - Gelding', jockey: "Liam O'Brien" },
  { gate: '04', horse: 'Desert Rose', breed: 'Arabian - Filly', jockey: 'Elena Rossi' },
  { gate: '05', horse: 'Iron Will', breed: 'Quarter Horse - Stallion', jockey: 'David Chen' },
  { gate: '06', horse: 'Silver Lining', breed: 'Thoroughbred - Mare', jockey: 'Sophie Martin' },
  { gate: '07', horse: 'Thunder Bolt', breed: 'Appaloosa - Gelding', jockey: 'James Wilson' },
  { gate: '08', horse: 'Velvet Night', breed: 'Arabian - Stallion', jockey: 'Aisha Khan' },
];

const initialIncidents: IncidentLog[] = [
  {
    id: 1,
    horse: 'Golden Sovereign',
    type: 'Interference',
    details: 'Crowding horse 02 at the 400m mark.',
    timestamp: '14:05:22 GMT',
  },
  {
    id: 2,
    horse: 'Midnight Breeze',
    type: 'Whip Violation',
    details: 'Excessive use of whip in final stretch.',
    timestamp: '14:08:45 GMT',
  },
];

const violationTypes = ['Interference', 'Whip Violation', 'False Start', 'Track Limits', 'Unsafe Riding', 'Equipment Issue'];

const titleCaseFromParam = (value?: string, fallback = 'Qualifier A') => {
...
export const RefereeRaceDetail: React.FC = () => {
  const { name } = useParams();
  const raceName = titleCaseFromParam(name);
  ...
  const heroMeta = useMemo(
    () => [
      { icon: <CalendarIcon />, label: 'Nov 12, 2024' },
      { icon: <ClockIcon />, label: '14:00 GMT' },
      { icon: <RulerIcon />, label: '1,200m' },
      { icon: <LocationIcon />, label: 'Heritage National Circuit' },
    ],
    [],
  );
    ...
    <RefereeShell>
      <section className="referee-race-detail">
        <section className="referee-race-hero-card" aria-label={`${raceName} race card`}>
          <img src={raceImage} alt="Race track banner" />
